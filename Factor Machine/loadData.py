...
def featureMatrix(ratingPath,moviePath,allGenre,movGen):
    ...
    data=np.array(data.drop(columns=['rating','timestamp','title','genres']))
    gen=np.zeros([len(data),len(allGenre)])
    for sample in range(len(data)):
        for g in movGen[data[sample][1]]:
    ...
    for userID in userItemDict:
        ...
        while negativeNum<ratio*positiveNum:
            ...
            if item not in positiveMov and item not in negativeMov:
                negativeMov=[userID,item]+genreOneHot(movGen[item],allGenre)+[-1]
...
def prepareData():
    ratingPath='/Users/user/git_repo/Movie-Recommender-System/movieLen/ratings.csv'
    moviePath='/Users/user/git_repo/Movie-Recommender-System/movieLen/movies.csv'
    ...
    negaSample=nSample(item_pool,userItemDict,allGenre,mov,2)
